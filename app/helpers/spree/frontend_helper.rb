module Spree
  module FrontendHelper
        ...
        taxons = root_taxon.children.map do |taxon|
         content_tag :li, class: '' do
            concat(link_to(taxon.name, seo_url(taxon))) 
                ...
                concat(("<ul class='second-level-menu'>").html_safe)
                   taxonc.each do |taxont|
                    concat(("<li>").html_safe)
                    concat(link_to(taxont.name, seo_url(taxont)))
                    taxontl = Spree::Taxon.where(:parent_id => taxont.id)
                    concat(("<ul class='third-level-menu'>").html_safe)
    ...
    def get_taxonid(taxon,current_taxon,max_level)
